...
class ServiceRequest(db.Model):
    ...
    date_of_request = db.Column(db.String(20))
...
def customer_dashboard():
    ...
    # Fetch all approved services provided by approved professionals
    services = (
        db.session.query(Service)
        #.join(ServiceProfessional, Service.professional_id == ServiceProfessional.id)
        .all()
        #.filter(ServiceProfessional.is_approved == True)
    )

    # Mark services as requested if they are already requested by the customer
    for service in services:
        service_request = ServiceRequest.query.filter_by(
            service_id=service.id,
            customer_id=current_user.id
        ).first()
        service.requested = bool(service_request)
        service.request_id = service_request.id if service_request else None
...
def request_service():
    ...
    if not service_id:
        flash('Service ID is missing.', 'error')
        return redirect(url_for('customer_dashboard'))
